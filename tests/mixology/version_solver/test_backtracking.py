...
def test_circular_dependency_on_older_version(root, provider, repo, f):
...
def test_diamond_dependency_graph(root, provider, repo, f):
...
def test_backjumps_after_partial_satisfier(root, provider, repo, f):
    ...
    root.add_dependency(f.create_dependency("c", "*"))
    root.add_dependency(f.create_dependency("y", "^2.0.0"))
...
def test_rolls_back_leaf_versions_first(root, provider, repo, f):
...
def test_simple_transitive(root, provider, repo, f):
...
def test_backjump_to_nearer_unsatisfied_package(root, provider, repo, f):
...
def test_traverse_into_package_with_fewer_versions_first(root, provider, repo, f):
...
def test_backjump_past_failed_package_on_disjoint_constraint(root, provider, repo, f):
    root.add_dependency(f.create_dependency("a", "*"))
